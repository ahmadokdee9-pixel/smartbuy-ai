...
export default function Home() {
  ...
  const stores = [
    { store: "Amazon", delivery: "Fast delivery", trust: "High", link: "https://www.amazon.nl/s?k=" },
    { store: "Bol.com", delivery: "1-2 days", trust: "High", link: "https://www.bol.com/nl/nl/s/?searchtext=" },
    { store: "MediaMarkt", delivery: "Store pickup", trust: "High", link: "https://www.mediamarkt.nl/nl/search.html?query=" },
    { store: "Coolblue", delivery: "Next day", trust: "High", link: "https://www.coolblue.nl/zoeken?query=" },
    { store: "IKEA", delivery: "Home delivery", trust: "High", link: "https://www.ikea.com/nl/en/search/?q=" },
    { store: "AliExpress", delivery: "Long delivery", trust: "Medium", link: "https://www.aliexpress.com/wholesale?SearchText=" },
    { store: "Temu", delivery: "Long delivery", trust: "New", link: "https://www.temu.com/search_result.html?search_key=" },
  ] as const;
  ...
  function getDecisionStyle(score: number) {
    ...
    if (score >= 55) return "bg-amber-400 text-black";
    return "bg-red-500 text-white";
  }

  function getAdvice(score: number) {
    if (score >= 88) return "This is the best buying option right now.";
    if (score >= 72) return "Good value, but compare final delivery costs.";
    if (score >= 55) return "Price is average. Waiting may be smarter.";
    return "Not recommended now. Better offers may exist.";
  }

  function generateOffers(searchText: string) {
    const base = getBasePrice(searchText);
    const modifiers = [0.88, 0.93, 0.97, 1.02, 1.08, 0.82, 0.79];

    return stores.map((s, index) => {
      const price = Math.round(base * modifiers[index]);
      const oldPrice = Math.round(base * 1.12);
      const rating = Number((4.0 + (index % 5) * 0.15).toFixed(1));

      return {
        id: index + 1,
        product: searchText || "product",
        store: s.store,
        price,
        oldPrice,
        rating,
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#060815] text-white">
      <div className="absolute inset-0 -z-10">
        ...
        <div className="absolute left-0 right-0 top-28 h-[420px] opacity-30 bg-[radial-gradient(ellipse_at_center,rgba(34,211,238,0.18),transparent_65%)]" />
      </div>
      ...
      <section className="max-w-6xl mx-auto px-6 pt-20 text-center">
        ...
        <p className="max-w-2xl mx-auto text-white/65 text-lg mb-8">
          Search any product and Smart Buy AI compares multiple stores, prices,
          trust, delivery, and tells you where to buy, wait, or avoid.
        </p>
        ...
        <div className="flex justify-center gap-3 flex-wrap mt-6 text-sm">
          {["iphone 15", "gaming laptop", "sofa", "shoes", "coffee machine", "perfume", "air fryer"].map((item) => (
            <button
              key={item}
              onClick={() => {
                setQuery(item);
                setTimeout(() => {
                  const generated = generateOffers(item).sort(
                    (a, b) => getDealScore(b) - getDealScore(a)
                  );
                  setOffers(generated);
                  setBest(generated[0]);
          ...
          <div className="bg-white/10 border border-white/15 rounded-[34px] p-8 backdrop-blur-2xl">
            <div className="flex flex-col md:flex-row justify-between gap-6">
              ...
              <div className="bg-black/35 border border-white/10 rounded-3xl p-6 min-w-[210px]">
                ...
                <p className={`mt-3 inline-block px-3 py-1 rounded-full font-black ${getDecisionStyle(getDealScore(best))}`}>
            ...
            <div className="mt-6 bg-cyan-400/10 border border-cyan-300/20 rounded-2xl p-4">
              ...
              <p className="text-white/70 mt-1">
                {getAdvice(getDealScore(best))} Smart Buy AI selected this store because it has the best balance of price, rating, trust, and delivery.
              </p>
            </div>
      ...
      <section className="max-w-6xl mx-auto px-6 mt-24 grid md:grid-cols-3 gap-5">
        ...
        <div className="bg-white/10 border border-white/10 rounded-3xl p-6">
          ...
          <p className="text-white/60 mt-2">Amazon, Bol, MediaMarkt, Coolblue, IKEA, AliExpress, Temu</p>
        </div>
      </section>
      ...
      <footer className="text-center text-white/40 py-12 mt-20">
        ...
        <p className="mt-1">Prototype now. Real store feeds and affiliate links will be connected next.</p>
      </footer>
    </main>
  );
}